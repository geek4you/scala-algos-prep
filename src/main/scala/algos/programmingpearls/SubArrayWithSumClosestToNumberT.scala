...
/**
 ...
 Let prefix[i] be the cumulative sum up to i, like in the example above.
Then, we need to compute the best subarray ending at i. This means we need to find an index j<i such that the sum is as close as possible to tt. This means that this value needs to be as close as possible to prefix[i] - t, since  prefix[i] - prefix[j] is the sum of the subsequence between [j,i]. So, for every prefix, we must find another earlier prefix that’s as close in value to prefix[i] - t. We can use a Java TreeSet for this which supports O(log(n))O(log⁡(n)) insertion, lower bound, and upper bound operations.
